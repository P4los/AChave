...
@password.post(
    "/", response_model=PasswordResponse, status_code=status.HTTP_201_CREATED
)
def create_password(
    ...
    if password_in.vault_id is None:
        ...
        if not default_vault:
            raise HTTPException(
                status_code=500, detail="Error interno: No tienes cofre por defecto."
            )
        ...
        if not any(v.vault_id == password_in.vault_id for v in db_vaults):
            raise HTTPException(
                status_code=403, detail="Cofre no autorizado o no existe."
            )
...
def update_password(
    ...
    updated_pwd = crud_pwd.update_password(
        db=db, password_id=password_id, password_in=password_in
    )
    if not updated_pwd:
        raise HTTPException(status_code=404, detail="Contraseña no encontrada")
    return updated_pwd
