...
export default function ExtensionHeader({ onOpenVaultModal }: { onOpenVaultModal: () => void }) {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width: '100%', background: 'white', padding: '8px 14px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
          <div style={{ width: 20, height: 20, borderRadius: 5, background: '#16A34A', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <ShieldCheck style={{ width: 12, height: 12, color: 'white' }} />
          </div>
          <span style={{ fontSize: 12, fontWeight: 800, color: '#0F172A', fontFamily: 'Inter, sans-serif' }}>AChave</span>
        </div>
      </div>
          ...
          <div style={{
            ...
            <div style={{ maxHeight: 200, overflowY: 'auto' }}>
              {vaults.map(v => {
                ...
                return (
                  <div key={v.vault_id} style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    borderBottom: '1px solid #F1F5F9',
                  }}>
                    <button onClick={() => { setSelectedVault(v); setOpen(false); }} style={{
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{
                          ...
                          <VI style={{ width: 12, height: 12, color: 'white' }} />
                        </div>
                        <span style={{ fontSize: 12, fontWeight: sel ? 700 : 600, color: sel ? '#0F172A' : '#475569', fontFamily: 'Inter, sans-serif' }}>
                          {v.name}
                        </span>
                      </div>
                      {sel && <Check style={{ width: 14, height: 14, color: '#16A34A' }} />}
                    </button>

                    {!v.is_default && (
                      <button onClick={(e) => { e.stopPropagation(); setDel(v); setOpen(false); }} style={{
                        padding: 8, background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8',
                      }}>
            ...
            <div style={{ padding: 6, borderTop: '1px solid #F1F5F9', background: '#F8FAFC' }}>
              <button onClick={() => { setOpen(false); onOpenVaultModal(); }} style={{
                width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4,
                padding: '6px 8px', borderRadius: 8, background: 'white', border: '1px solid #E2E8F0',
                color: '#334155', fontWeight: 700, fontSize: 11, cursor: 'pointer', fontFamily: 'Inter, sans-serif',
              }}>
