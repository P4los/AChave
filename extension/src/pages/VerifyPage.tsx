...
export default function VerifyPage() {
  ...
  useEffect(() => {
    ...
    if (!t) {
      setVerifyError("Enlace inválido.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (masterKey.length < 8) {
      setSetupError("Mínimo 8 caracteres.");
      return;
    }
    ...
    try {
      ...
      const enc = async (t: string) => {
        ...
        const c = new Uint8Array(iv.length + buf.byteLength);
        c.set(iv, 0);
        c.set(new Uint8Array(buf), iv.length);
        return uint8ArrayToBase64(c);
      };
      await setupCrypto(
        {
          password: masterKey,
          validador_cifrado: await enc("SESAMO_ABIERTO"),
          llave_publica: pub,
          llave_privada_cifrada: await enc(priv),
        },
        authToken,
      );
      await setKeys({ pub, priv });
      await fetchVaults();
      navigate("/claves");
    } catch (e: any) {
      ...
      >
        {isVerifying ? (
          ...
          >
            ...
            <span style={{ fontSize: 12, color: "var(--ext-text-secondary)" }}>
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor:
                    isSubmitting || masterKey.length < 8
                      ? "not-allowed"
                      : "pointer",
                  fontSize: 14,
                  background:
