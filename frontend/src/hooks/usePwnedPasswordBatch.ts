...
export function usePwnedPasswordBatch() {
  ...
  const checkBatchVulnerabilities = useCallback(async (passwordsToCheck: { id: string, plainText: string }[]) => {
    ...
    try {
      // Usamos una concurrencia limitada (Promise.all) en lotes pequeños
      // para no saturar la API ni el navegador si hay cientos de claves
      const batchSize = 10;
      for (let i = 0; i < passwordsToCheck.length; i += batchSize) {
        ...
        await Promise.all(batch.map(async ({ id, plainText }) => {
          if (!plainText || plainText.length < 8) {
            results[id] = false; // Ignoramos cortas, o asumimos no-pwned por esta API
            return;
          }
          ...
          try {
            ...
            results[id] = false; // Fallback "no vulnerable" si falla la API individual
          }
        }));
      }
    } catch (err) {
