...
export function MobileHeader({
  ...
  return (
    <div className="md:hidden flex items-center justify-between w-full min-h-16 bg-white dark:bg-slate-950 px-5 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-40">
      ...
      <div className="relative flex-1 py-3">
        ...
        {isDropdownOpen && vaults.length > 0 && (
          <div className="absolute top-full left-0 mt-2 w-[240px] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-[16px] shadow-2xl overflow-hidden z-20">
            <div className="max-h-[50vh] overflow-y-auto">
              {vaults.map((vault) => {
                ...
                return (
                  <div
                    key={vault.vault_id}
                    className="w-full flex items-center justify-between hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors border-b border-slate-100 dark:border-slate-800 last:border-b-0"
                  >
                    <button
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span
                          className={`text-[14px] font-bold max-w-30 ${isSelected ? "text-slate-900 dark:text-slate-100" : "text-slate-600 dark:text-slate-300"}`}
                        >
                          {vault.name}
                        </span>
                      </div>
            ...
            <div className="p-2 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-900">
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
              >
